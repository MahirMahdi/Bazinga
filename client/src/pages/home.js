    ...
    async function getConversation(){
        ...
        if(conversations?.length > 0){
            conversations.map((convo)=>(
                socket.emit('join-room',convo._id)
            ))
        ...
        <div className="home">
            ...
            <div className="sidebar-mobile">
                <div className="chatcard-holder">
                    {result?<ChatCard status={onlineUsers?.filter(user => user.userId === result._id).length !== 0? true : false} searchData={result} currentUserId={user.user._id} data={null}/>: conversation? conversation.map((convo)=>{
                        const userId = convo.members.filter(id=> id !== user.user._id)[0]
                        const activeUser = onlineUsers?.filter(user => user.userId === userId)
                        return(<ChatCard key={convo._id} userId={userId} status={activeUser?.length !== 0? true : false} searchData={null} currentUserId={user.user._id} data={convo}/>)
                    }): null}
                </div>
            </div>
